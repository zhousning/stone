class DesignerElectricCertsController < ApplicationController
  ...
  def index
    @designer_electric_certs = DesignerElectricCert.all
  end
  ...
  def create
    ...
    #@designer_electric_cert.user = current_user
    if @designer_electric_cert.save
      redirect_to @designer_electric_cert
    else
      render :new
    end
  end
  ...
  def edit
    @designer_electric_cert = DesignerElectricCert.find(params[:id])
  end
  ...
  def update
    ...
    if @designer_electric_cert.update(designer_electric_cert_params)
      redirect_to designer_electric_cert_path(@designer_electric_cert) 
    else
      render :edit
    end
  end
